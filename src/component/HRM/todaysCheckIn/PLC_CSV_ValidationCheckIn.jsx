...
const PLC_CSV_ValidationCheckIn = ({selectedDate}) => {
    const {plcCsvCheckInSiteData,
      plcCsvCheckInOfficeData,
      fetchPlcCsvCheckInSiteData,
      fetchPlcCsvCheckInOfficeData,} = useStateContext()

      useEffect(() => {
        const fetchData = async () => {
          await fetchPlcCsvCheckInSiteData();
          await fetchPlcCsvCheckInOfficeData();
        };
    
        fetchData();
      }, []);
    
      const filteredOfficeReports = plcCsvCheckInOfficeData.filter(
        (officeReport) => officeReport.date === selectedDate
      );
      const filteredSiteReports = plcCsvCheckInSiteData.filter(
...
export default PLC_CSV_ValidationCheckIn