...
const SteamQualityTest = ({selectedDate}) => {

  const {steamCheckInSiteData,
    steamCheckInOfficeData,
    fetchSteamCheckInSiteData,
    fetchSteamCheckInOfficeData,} = useStateContext()

  useEffect(() => {
    const fetchData = async () => {
      await fetchSteamCheckInSiteData();
      await fetchSteamCheckInOfficeData();
    };

    fetchData();
  }, []);

  const filteredOfficeReports = steamCheckInOfficeData.filter(
    (officeReport) => officeReport.date === selectedDate
  );
  const filteredSiteReports = steamCheckInSiteData.filter(
...
export default SteamQualityTest