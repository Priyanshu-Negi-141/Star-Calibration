...
const CAValidation = ({selectedDate}) => {

    const {caCheckInSiteData,
      caCheckInOfficeData,
      fetchCaCheckInSiteData,
      fetchCaCheckInOfficeData,} = useStateContext()

      useEffect(() => {
        const fetchData = async () => {
          await fetchCaCheckInSiteData();
          await fetchCaCheckInOfficeData();
        };
    
        fetchData();
      }, []);
    
      const filteredOfficeReports = caCheckInOfficeData.filter(
        (officeReport) => officeReport.date === selectedDate
      );
      const filteredSiteReports = caCheckInSiteData.filter(
  ...
  return (
    <div>
      <div>
        <h3 className="text-bold underline">CA Office Data</h3>
        ...
        <h3 className="text-bold underline">CA Site Data</h3>
