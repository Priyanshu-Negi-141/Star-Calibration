...
const ThermalValidationCheckIN = ({selectedDate}) => {
    const {thermalCheckInSiteData,
      thermalCheckInOfficeData,
      fetchThermalCheckInSiteData,
      fetchThermalCheckInOfficeData,} = useStateContext()

      useEffect(() => {
        const fetchData = async () => {
          await fetchThermalCheckInSiteData();
          await fetchThermalCheckInOfficeData();
        };
    
        fetchData();
      }, []);
    
      const filteredOfficeReports = thermalCheckInOfficeData.filter(
        (officeReport) => officeReport.date === selectedDate
      );
      const filteredSiteReports = thermalCheckInSiteData.filter(
...
export default ThermalValidationCheckIN