...
import { employeeListData } from "../../../data/userManagement/employeeListData";
import { useStateContext } from "../../../contexts/ContextProvider";

const CalibrationCheckIN = ({ selectedDate }) => {
  const {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await fetchCalibrationCheckInSiteData();
      await fetchCalibrationCheckInOfficeData();
    };

    fetchData();
  }, []);

  const filteredOfficeReports = calibrationCheckInOfficeData.filter(
    (officeReport) => officeReport.date === selectedDate
  );
  const filteredSiteReports = calibrationCheckInSiteData.filter(
...
export default CalibrationCheckIN;
